...
// NewFixWindowLimiter 固定窗口算法
func NewFixWindowLimiter(window time.Duration, threshold int) *FixWindowLimiter {
...
func (c *FixWindowLimiter) BuildServerInterceptor() grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (resp any, err error) {
		c.lock.Lock()
		now := time.Now()
